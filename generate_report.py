...
shutil.copytree(os.path.join(forbok.script_root, 'templates', 'report_skeleton'), os.path.join(args.output, args.period), dirs_exist_ok=True)
...
payload = { 'title' : f"Förenklat bokslut - {config['name']}",
            'name'  : f"{config['name']} - {args.period}"
            }

# render template
content = template.render(payload)
with open(os.path.join(forbok.script_root, args.output, args.period, 'index.html'), 'w') as html_file:
